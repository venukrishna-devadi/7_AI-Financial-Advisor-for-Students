...
class FinancialGraphState(TypedDict, total=False):
    """
    Shared state object passed across the graph.

    Each node reads what it needs and writes back only its output.
    """
...
class FinancialGraphBuilder:
    ...
    def _append_warning(self, state: FinancialGraphState, message: str) -> List[str]:
        """
        Return a new warnings list with one extra warning appended.
        """
    ...
    def _append_error(self, state: FinancialGraphState, message: str) -> List[str]:
        """
        Return a new errors list with one extra error appended.
        """
    ...
    def _route_after_intake(self, state: FinancialGraphState) -> str:
        ...
        Rules:
        1. if intake marked pipeline as failed -> finalize
        2. if there are no transactions -> finalize
        3. otherwise -> analyzer
        """
        if state.get("pipeline_status") == "failed":
    ...
    def intake_node(self, state: FinancialGraphState) -> Dict[str, Any]:
        """
        Normalize incoming data into the graph.
        ...
        # Case 1: transactions already exist
        if transactions:
            updates["transactions"] = transactions
            updates["extraction_result"] = {
                "mode": "prebuilt_transactions",
        ...
        # Case 2: bridge vision output into Transaction objects
        if vision_output:
            bridge_result = vision_result_to_transactions(
                vision_output=vision_output,
                student_id=student.student_id,
            )

            updates["transactions"] = bridge_result.transactions
            updates["extraction_result"] = {
                "mode": "vision_bridge",
                **bridge_result.to_dict(),
            }

            warnings = list(state.get("warnings", []) or [])
            errors = list(state.get("errors", []) or [])

            if bridge_result.skipped_items:
                warnings.append(
                    f"Skipped {len(bridge_result.skipped_items)} extracted item(s) during vision-to-transaction conversion."
                )

            if bridge_result.errors:
                errors.extend(bridge_result.errors)
        ...
        updates["warnings"] = self._append_warning(
            state,
            "No transactions or vision output provided. Continuing with empty transaction history.",
        )
        updates["extraction_result"] = {
            "mode": "empty_input",
            "transaction_count": 0,
    ...
    def analyzer_node(self, state: FinancialGraphState) -> Dict[str, Any]:
        """
        Analyze the student's transaction history to identify:
        - spending patterns
        - trends
        - anomalies
        - budget insights
    ...
    def planner_node(self, state: FinancialGraphState) -> Dict[str, Any]:
        """
        Build a financial plan using student profile, transactions, and analysis.
        ...
        if student is None:
            return {
                "errors": self._append_error(
                    state,
                    "Planner node could not run because student is missing.",
                )
            }
    ...
    def tracker_node(self, state: FinancialGraphState) -> Dict[str, Any]:
        ...
        Tracker is allowed to run even when budget is missing,
        because it can still provide partial status and goal monitoring.
        """
        student = state.get("student")
        transactions = state.get("transactions", []) or []
        budget = state.get("budget")
    ...
    def advisor_node(self, state: FinancialGraphState) -> Dict[str, Any]:
        ...
        tracking_report = state.get("tracking_report", {}) or {}
        goals = state.get("goals", []) or []
    ...
    def finalize_node(self, state: FinancialGraphState) -> Dict[str, Any]:
        ...
        if errors:
            return {"pipeline_status": "completed_with_errors"}
    ...
    def run(self, initial_state: FinancialGraphState) -> FinancialGraphState:
        """
        Main entry point most callers should use.
        ...
        if validation_errors:
            return {
                **initial_state,
                "warnings": list(initial_state.get("warnings", []) or []),
                "errors": list(initial_state.get("errors", []) or []) + validation_errors,
                "pipeline_status": "failed",
            }
