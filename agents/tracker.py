...
class TrackingAlert:
    """
    Small alert object used internally, then converted to dict.
    ...
    def to_dict(self) -> Dict[str, Any]:
        """
        Convert dataclass to clean JSON-friendly dict.
        Ensure data is always a dictionary.
        """
        d = asdict(self)
        if d["data"] is None:
...
class TrackerAgent:
    ...
    def track_student(
        ...
        *,
        ...
        Flow:
        1. Filter transactions to the budget window if budget exists
        2. Track budget overall + by category
        3. Track goals (if any)
        4. Generate alerts from the current status
        5. Generate recommendations
        ...
        if budget is not None:
            ...
            budget_txns = list(transactions)
            budget_report = self._empty_budget_report()
            budget_period_payload = None
        ...
        return {
            "student_id": student.student_id,
            "tracking_date": tracking_date.isoformat(),
            "budget_period": budget_period_payload,
            "budget_status": budget_report["overall"],
            "category_tracking": budget_report["categories"],
            "goal_tracking": goal_report,
            "alerts": [a.to_dict() for a in alerts],
    ...
    def _filter_transactions_for_budget_period(
        self,
        transactions: List[Transaction],
        budget: Budget,
    ) -> List[Transaction]:
    ...
    def _empty_budget_report(self) -> Dict[str, Any]:
        """
        Return a safe empty budget report when no budget exists.
        """
        return {
            "overall": {
                "total_budget": 0.0,
                ...
                "pace": {
                    "elapsed_days": 0,
                    "total_days": 0,
                    "expected_used_percent_by_now": 0.0,
    ...
    def _track_budget(
        ...
        *,
        ...
        ahead_count = sum(
            1 for c in category_tracking.values()
            if c.get("pace", {}).get("pace") == "ahead_of_spend"
        )
        if ahead_count >= 2 and overall_status == "healthy":
            overall_status = "warning"
    ...
    def _calculate_budget_pace(
        ...
        *,
        ...
        Example:
        - if 50% of the month is passed but 80% of the budget is gone -> ahead_of_spend
        - if 50% of the month has passed and 40% is used -> under_pace/on_track
        """
        total_days = max(1, (end_date - start_date).days + 1)
        elapsed_days = min(max(1, (tracking_date - start_date).days + 1), total_days)
    ...
    def _infer_savings_from_transactions(
        self,
        transactions: List[Transaction],
        goal: Goal,
    ) -> float:
        """
        Best-effort helper to detect savings-like contributions from transactions.
        Currently not heavily used, but kept for future expansion.
        """
        keywords = ["savings", "transfer to savings", "to savings"]
        savings_txns = [
            t for t in transactions
            if self._is_expense(t) and any(k in (t.description or "").lower() for k in keywords)
        ]
        return sum(float(t.amount) for t in savings_txns)
    ...
    def _track_goals(
        ...
        *,
        ...
        Simple logic:
        - use goal.current_amount as source of truth
        - compute progress, remaining, status
        - estimate pace if target_date exists
        """
        reports: List[Dict[str, Any]] = []
    ...
    def _calculate_goal_pace(
        self,
        goal: Goal,
        tracking_date: date,
    ) -> Dict[str, Any]:
        """
        Compare actual goal progress vs expected progress by this date.
        """
    ...
    def _generate_alerts(
        ...
        *,
        ...
        # Overall pace alert should be added once, not once per category
        overall_pace = overall.get("pace", {})
        if overall_pace.get("pace") == "ahead_of_spend" and overall["status"] != "critical":
            alerts.append(
                TrackingAlert(
                    message="Overall budget is being spent faster than expected for this point in the cycle.",
                    severity="warning",
        ...
        for cat, info in categories.items():
            if info["status"] == "over_budget":
                ...
                alerts.append(
                    TrackingAlert(
                        message=f"'{cat}' is nearing its limit ({info['percent_used']}% used).",
                        severity="warning",
                        category=cat,
                        data=info,
                    )
                )
            elif info.get("pace", {}).get("pace") == "ahead_of_spend":
                alerts.append(
                    TrackingAlert(
                        message=f"'{cat}' is being spent faster than expected for this point in the cycle.",
                        severity="warning",
                        category=cat,
                        data=info,
                    )
        ...
        for goal in goal_report:
            ...
            pace = goal.get("pace")
            if pace and pace.get("pace") == "behind":
                alerts.append(
                    TrackingAlert(
                        message=f"Goal '{goal_name}' is behind schedule.",
                        severity="warning",
    ...
    def _generate_recommendations(
        ...
        *,
        ...
        """
        Convert tracking state into short, actionable recommendations.
        """
        recommendations: List[str] = []
        ...
        if overall["status"] == "critical":
            recommendations.append("Pause excess discretionary spending until budget pressure drops.")
        elif overall["status"] == "warning":
            recommendations.append("Review this week's spending and trim at least one non-essential category.")

        for cat, info in categories.items():
            if info["status"] == "over_budget":
                recommendations.append(f"Stop or sharply reduce spending in '{cat}' for the rest of this budget period.")
            elif info["status"] == "warning":
                recommendations.append(f"Be careful with '{cat}' — it is close to the budget limit.")
            elif info.get("pace", {}).get("pace") == "ahead_of_spend":
                recommendations.append(f"Slow down spending in '{cat}' to avoid going over budget later this period.")
