...
class PlanResult:
    ...
    def to_dict(self) -> Dict[str, Any]:
        budget_dict = None
        if self.recommended_budget:
            budget_dict = self.recommended_budget.model_dump(mode="json")

        goals_list = [g.model_dump(mode="json") for g in self.suggested_goals]
...
class PlannerAgent:
    """
    PlannerAgent turns analysis into an action plan.
    ...
    def __init__(self, *, name: str = "Planner Agent"):
        ...
        self.fixed_categories = {
            ...
            "insurance", "tuition", "student_loan", "car_payment", "credit_card"
        }
        ...
        self.flexible_categories = {
            ...
            "personal_care", "travel", "flights", "hotels"
        }
    ...
    def build_plan(
        ...
        *,
        ...
        monthly_income = float(baseline.get("monthly_income", 0.0))
        fixed = float(baseline.get("fixed_monthly_expenses", 0.0))
        variable = float(baseline.get("variable_spend_est_monthly", 0.0))
        total_spend = float(baseline.get("total_spend_est_monthly", 0.0))

        if monthly_income <= 0:
            warnings.append("Monthly income is 0 — planning quality is limited until income is provided.")

        if monthly_income > 0 and fixed > monthly_income:
            warnings.append("Fixed expenses exceed monthly income. First priority is reducing bills or increasing income.")

        if monthly_income > 0 and total_spend > monthly_income:
            excess = total_spend - monthly_income
            warnings.append(
                f"Estimated monthly spending exceeds income by about ${excess:.0f}. Focus on stabilization before aggressive savings."
            )

        savings_plan = self._recommend_savings(
            baseline=baseline,
            student=student,
            analysis=analysis,
        )
    ...
    def _compute_baseline(self, student: Student, transactions: List[Transaction]) -> Dict[str, Any]:
        """
        Safer baseline:
        - monthly_income comes from student profile
        - fixed expenses come from student profile
        - variable spend uses recent expense behavior
        - disposable_after_fixed = income - fixed
        - disposable_after_estimated_spend = income - fixed - variable
        """
        monthly_income = float(getattr(student, "monthly_income", 0.0) or 0.0)
        fixed_expenses = sum(float(v) for v in getattr(student, "fixed_monthly_expenses", {}).values())
        ...
        # monthlyized recent expense estimate
        variable_monthly = (total_expenses / window_days) * 30.44 if window_days > 0 else 0.0

        total_spend_monthly = fixed_expenses + variable_monthly
        disposable_after_fixed = monthly_income - fixed_expenses
        disposable_after_estimated_spend = monthly_income - total_spend_monthly
        ...
        return {
            ...
            "total_spend_est_monthly": round(total_spend_monthly, 2),
            "window_days_used": window_days,
            "disposable_after_fixed": round(disposable_after_fixed, 2),
            "disposable_after_estimated_spend": round(disposable_after_estimated_spend, 2),
            "notes": "Baseline uses student.monthly_income plus recent expense pace to estimate current monthly behavior.",
        }

    def _recommend_savings(
        self,
        baseline: Dict[str, Any],
        student: Student,
        analysis: Dict[str, Any],
        ...
        """
        Safer savings logic:
        - use disposable_after_estimated_spend, not just income - fixed
        - if currently overspending, set savings target to 0
        - keep target modest and realistic
        """
        disposable_after_spend = float(baseline.get("disposable_after_estimated_spend", 0.0))
        monthly_income = float(baseline.get("monthly_income", 0.0))
        ...
        if monthly_income <= 0:
            return {
                "target_monthly_savings": 0.0,
                "strategy": "none",
                "rationale": "Income is missing, so a savings target would be unreliable.",
            }

        if disposable_after_spend <= 0:
            return {
                "target_monthly_savings": 0.0,
                "strategy": "stabilize_first",
                "rationale": "Current estimated monthly spending already uses up available income. Reduce spending first, then add savings.",
            }

        risk = str(getattr(student, "risk_profile", "moderate")).lower()
        pct = 0.10
        if risk == "moderate":
            pct = 0.15
        ...
        # keep it modest: max 25% of disposable_after_spend
        target = disposable_after_spend * pct
        target = min(target, disposable_after_spend * 0.25)

        target_rounded = self._round_nice(target)

        return {
            "target_monthly_savings": round(target_rounded, 2),
            "strategy": f"{int(pct * 100)}% of remaining cash flow after estimated spending (risk_profile={risk})",
            "rationale": "Savings target is based on realistic remaining cash flow after fixed expenses and recent variable spending.",
        }
    ...
    def _recommend_budget(
        ...
        *,
        ...
        expenses = [t for t in transactions if self._is_expense(t)]
        if not expenses:
            return None, {"reason": "No expense data available to build a budget."}
        ...
        for t in expenses:
            cat = str(t.category)
            spend_by_cat[cat] += float(t.amount)
            count_by_cat[cat] += 1
        ...
        monthly_by_cat = {
            cat: spend * multiplier
            for cat, spend in spend_by_cat.items()
        }

        limits: Dict[str, float] = {
            cat: self._round_nice(val)
            for cat, val in monthly_by_cat.items()
        }
        ...
        # Only nudge concentration if category is flexible.
        if concentration_cat and concentration_cat in limits and concentration_cat in self.flexible_categories:
            drop = 0.10 if float(concentration_pct or 0) >= 80 else 0.05
            old = limits[concentration_cat]
            limits[concentration_cat] = self._round_nice(max(0.0, old * (1 - drop)))
            nudges.append(f"Reduced '{concentration_cat}' limit by {int(drop * 100)}% due to high recent concentration.")

        if weekend_spike:
            for cat in ("entertainment", "dining_out", "games", "movies", "coffee"):
                if cat in limits:
                    old = limits[cat]
                    limits[cat] = self._round_nice(max(0.0, old * 0.90))
                    nudges.append(f"Nudged '{cat}' down by 10% due to weekend spending pattern.")
        ...
        if proposed_total > capacity and proposed_total > 0 and capacity > 0:
            scale = capacity / proposed_total
            for cat in list(limits.keys()):
                limits[cat] = self._round_nice(max(0.0, limits[cat] * scale))
            scaled = True
            nudges.append("Scaled all category limits down to fit income after fixed expenses and savings target.")
        ...
        if not budget_categories:
            return None, {"reason": "No valid categories found to build a budget."}
        ...
        recommended = Budget(
            ...
            savings_goals=savings_target if savings_target > 0 else None,
            alert_threshold=0.8,
            notes="Auto-generated by PlannerAgent from recent spending and analysis nudges.",
        )
    ...
    def _suggest_goals(
        ...
        """
        Safer starter goal:
        - suggest 1 month of FIXED expenses, not total noisy spend
        - minimum fallback 500
        """
        if existing_goals:
            return []

        fixed_monthly = float(baseline.get("fixed_monthly_expenses", 0.0))
        target = self._round_nice(fixed_monthly if fixed_monthly > 0 else 500.0)
        contrib = float(savings_plan.get("target_monthly_savings", 0.0))
        ...
        goal = Goal(
            ...
            current_amount=0.0,
            target_date=None,
            recurring_type=RecurringType.MONTHLY if contrib > 0 else RecurringType.ONE_TIME,
            recurring_amount=contrib if contrib > 0 else None,
            priority=GoalPriority.HIGH,
            notes="Auto-suggested goal: build a basic emergency cushion.",
        )
    ...
    def _build_action_plan(
        ...
        *,
        ...
        if monthly_income > 0 and fixed > monthly_income:
            actions.append(
                PlanAction(
                    title="Reduce fixed bills or increase income",
                    description="Your fixed monthly expenses are greater than your income. Start by reviewing housing, insurance, phone, internet, loan, or car obligations first.",
                    priority="high",
                    impact_monthly_usd=0.0,
                    confidence="high",
                    tags=["fixed_expenses", "critical"],
        ...
        if savings_target > 0 and disposable_after_spend > 0:
            actions.append(
                PlanAction(
                    title="Automate savings",
                    description=f"Set an automatic transfer of about ${savings_target:.0f}/month right after payday.",
                    priority="high",
                    impact_monthly_usd=savings_target,
                    confidence="high",
                    tags=["savings"],
        ...
        subs = self._extract_subscriptions_from_analysis(analysis)
        if subs:
            est = min(30.0, sum(float(s.get("approx_amount", 0.0)) for s in subs[:2]) or 15.0)
            actions.append(
                PlanAction(
                    title="Review subscriptions",
                    description=f"Review recurring charges like {', '.join(s.get('merchant', '') for s in subs[:3])}. Cancel or downgrade anything you do not use often.",
                    priority="medium",
                    impact_monthly_usd=est,
                    confidence="medium",
                    tags=["subscriptions"],
        ...
        for p in patterns:
            ...
            if p.get("name") == "category_concentration":
                ...
                if cat in self.flexible_categories:
                    actions.append(
                        PlanAction(
                            title="Reduce your biggest flexible spending bucket",
                            description=f"Recent spending is heavily concentrated in '{cat}' ({pct:.0f}%). Try a 5–10% reduction this month.",
                            priority="high",
                            impact_monthly_usd=0.0,
                            confidence="medium",
                            tags=["spending_pattern", "category"],
            ...
            if p.get("name") == "weekend_spending":
                actions.append(
                    PlanAction(
                        title="Add a weekend spending cap",
                        description="You tend to spend more on weekends. Set a small weekend cap and plan one low-cost activity.",
                        priority="medium",
                        impact_monthly_usd=0.0,
                        confidence="medium",
                        tags=["spending_pattern", "weekend"],
        ...
        if recommended_budget:
            actions.append(
                PlanAction(
                    title="Track spending weekly against your budget",
                    description="Check your budget once a week and make small corrections before the month gets away from you.",
                    priority="medium",
                    impact_monthly_usd=0.0,
                    confidence="high",
                    tags=["budget"],
        ...
        if suggested_goals:
            ...
            actions.append(
                PlanAction(
                    ...
                    description=f"Start with a goal of ${g.target_amount:.0f}. Even small monthly progress builds stability.",
                    priority="medium",
                    impact_monthly_usd=float(g.recurring_amount or 0.0),
                    confidence="high",
                    tags=["goals", "emergency_fund"],
    ...
    def _build_projections(
        ...
        *,
        ...
        """
        Keep projections simple and not overconfident.
        Use only savings target plus explicitly estimated non-goal action impacts.
        """
        savings_target = float(savings_plan.get("target_monthly_savings", 0.0))
        non_goal_impacts = sum(
            float(a.impact_monthly_usd or 0.0)
            for a in action_plan
            if "goals" not in (a.tags or [])
        )

        monthly_total = max(0.0, non_goal_impacts)
    ...
    # Small utilities
    # -------------------------
    def _extract_subscriptions_from_analysis(self, analysis: Dict[str, Any]) -> List[Dict[str, Any]]:
        patterns = analysis.get("patterns") or []
        for p in patterns:
            if isinstance(p, dict) and p.get("name") == "subscriptions":
                data = p.get("data") or {}
    ...
    def _round_nice(self, value: float) -> float:
        v = float(value or 0.0)
        if v <= 0:
            return 0.0
        if v < 50:
