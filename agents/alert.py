...
class AlertAgent:
    ...
    def __init__(self, name: str = "Alert Agent"):
        ...
        self.fixed_expense_categories = {
            "housing", "rent", "utilities", "internet", "phone", "tuition", "insurance",
            "student_loan", "car_payment", "credit_card", "mortgage"
        }
        self.flexible_expense_categories = {
            "food", "groceries", "dining_out", "coffee",
            "entertainment", "streaming", "games", "movies",
    ...
    def _budget_alerts(self, tracking_report: Dict[str, Any]) -> List[FinancialAlert]:
        ...
        # If there is no real budget, do not produce budget stress alerts.
        if budget_status.get("status", "unknown") == "unknown":
            return alerts

        overall_status = budget_status.get("status", "healthy")
        total_percent = float(budget_status.get("percent_used", 0) or 0)
        total_budget = float(budget_status.get("total_budget", 0) or 0)
        total_spent = float(budget_status.get("total_spent", 0) or 0)
            ...
            if overall_status == "critical":
                alerts.append(
                    self._make_alert(
                        ...
                        recommended_action="Pause non-essential spending immediately and review this week's purchases.",
                        source="tracker",
                        metadata={
                            "percent_used": total_percent,
                            "total_budget": total_budget,
                ...
                alerts.append(
                    self._make_alert(
                        ...
                        recommended_action="Reduce discretionary spending this week and protect the remaining budget.",
                        source="tracker",
                        metadata={
                            "percent_used": total_percent,
                            "total_budget": total_budget,
        ...
        for category, info in category_tracking.items():
            status = info.get("status", "on_track")
            percent_used = float(info.get("percent_used", 0) or 0)
            spent = float(info.get("spent", 0) or 0)
            limit = float(info.get("limit", 0) or 0)
            ...
            if status == "over_budget":
                alerts.append(
                    self._make_alert(
                        ...
                        severity="critical",
                        title=f"{category} is over budget",
                        message=f"Your {category} spending is at {percent_used:.1f}% (${spent:.2f} of ${limit:.2f}).",
                        recommended_action=self._recommended_category_action(category, "critical"),
                        source="tracker",
                        category=category,
                        metadata={
                ...
                alerts.append(
                    self._make_alert(
                        ...
                        severity="warning",
                        title=f"{category} is close to budget limit",
                        message=f"Your {category} spending is already at {percent_used:.1f}% (${spent:.2f} of ${limit:.2f}).",
                        recommended_action=self._recommended_category_action(category, "warning"),
                        source="tracker",
                        category=category,
                        metadata={
    ...
    def _pace_alerts(self, tracking_report: Dict[str, Any]) -> List[FinancialAlert]:
        ...
        overall_pace = budget_status.get("pace", {}) or {}
        overall_pace_status = overall_pace.get("pace", overall_pace.get("status", "on_track"))
        ...
        if overall_pace_status == "ahead_of_spend" and budget_status.get("status") == "healthy":
            alerts.append(
                self._make_alert(
                    ...
                    message=(
                        f"You have used {budget_status.get('percent_used', 0):.1f}% of your budget "
                        f"while expected usage by now is {overall_pace.get('expected_used_percent_by_now', 0):.1f}%."
                    ),
                    recommended_action="Slow down discretionary spending this week before the budget becomes stressed.",
                    source="tracker",
                    metadata=overall_pace,
        ...
        if len(ahead_categories) >= 3:
            alerts.append(
                self._make_alert(
                    ...
                    severity="warning",
                    title="Several categories are ahead of spending pace",
                    message=f"Multiple categories are moving too fast this month: {', '.join(ahead_categories[:5])}.",
                    recommended_action="Reduce optional spending across these categories before they become budget problems.",
                    source="tracker",
                    metadata={"ahead_categories": ahead_categories},
                )
    ...
    def _goal_alerts(
        ...
        if isinstance(goal_tracking, list) and goal_tracking:
            for g in goal_tracking:
                ...
                pace = g.get("pace", {}) or {}
                pace_status = pace.get("pace", pace.get("status", ""))
                ...
                if pace_status == "behind":
                    alerts.append(
                        self._make_alert(
                            ...
                            severity="warning",
                            title=f"{goal_name} is behind schedule",
                            message=f"Goal progress is {progress:.1f}% and is currently behind the expected pace.",
                            recommended_action="Increase contributions slightly or rebalance spending to protect this goal.",
                            source="goal",
                            goal_name=goal_name,
                            metadata={
            ...
            if days_left is not None and days_left <= 45 and progress < 50:
                alerts.append(
                    self._make_alert(
                        ...
                        severity="warning",
                        title=f"{goal.name} may miss its target date",
                        message=f"{goal.name} has only {days_left} days left and is at {progress:.1f}% progress.",
                        recommended_action="Increase contributions or revise the timeline before the deadline arrives.",
                        source="goal",
                        goal_name=goal.name,
                        metadata={
    ...
    def _savings_alerts(
        ...
        amount_earned = float(summary.get("amount_earned", 0) or 0)
        amount_spent = float(summary.get("amount_spent", 0) or 0)

        # Many screenshot/bank-statement flows do not classify credits as income correctly yet.
        # So we only trust savings-rate alerts when meaningful income data exists.
        has_reliable_income = amount_earned > 0

        if has_reliable_income:
            savings_rate = ((amount_earned - amount_spent) / amount_earned) * 100
            ...
            if amount_spent > amount_earned:
                alerts.append(
                    self._make_alert(
                        alert_type="negative_cashflow",
                        severity="critical",
                        title="Spending is higher than income",
                        message=f"Expenses (${amount_spent:.2f}) are currently above income (${amount_earned:.2f}).",
                        recommended_action="Cut discretionary spending immediately and stabilize the monthly plan before saving more.",
                        source="planner",
                        metadata={
                            "amount_earned": amount_earned,
        ...
        # Only show emergency-fund alerts if the profile actually has savings info.
        current_savings = getattr(student, "current_savings", None)

        if current_savings is not None and hasattr(student, "total_fixed_expenses"):
            fixed_exp = float(student.total_fixed_expenses())
                ...
                if months_covered < 1:
                    alerts.append(
                        self._make_alert(
                            ...
                            message=f"Current savings cover only about {months_covered:.1f} months of fixed expenses.",
                            recommended_action="Prioritize emergency savings until at least 1 month of fixed expenses is covered.",
                            source="planner",
                            metadata={"months_covered": round(months_covered, 1)},
                        )
                    ...
                    alerts.append(
                        self._make_alert(
                            ...
                            severity="info",
                            title="Emergency fund could be stronger",
                            message=f"Current savings cover about {months_covered:.1f} months of fixed expenses.",
                            recommended_action="Gradually increase emergency fund contributions toward a safer cushion.",
                            source="planner",
                            metadata={"months_covered": round(months_covered, 1)},
                        )
    ...
    def _stress_alerts(
        ...
        budget_status = tracking_report.get("budget_status", {}) or {}
        percent_used = float(budget_status.get("percent_used", 0) or 0)
        overall_status = budget_status.get("status", "unknown")

        baseline = plan.get("baseline", {}) or {}
        total_monthly_spend = float(baseline.get("total_spend_est_monthly", 0) or 0)
        monthly_income = float(getattr(student, "monthly_income", 0.0) or 0.0)

        if monthly_income > 0 and total_monthly_spend > monthly_income * 1.10:
            gap = total_monthly_spend - monthly_income
            alerts.append(
                self._make_alert(
                    alert_type="monthly_plan_under_stress",
            ...
            if monthly_income > 0 and disposable <= monthly_income * 0.05:
                alerts.append(
                    self._make_alert(
                        ...
                        severity="warning",
                        title="Monthly financial buffer is very small",
                        message=f"Only about ${disposable:.2f} remains after fixed monthly expenses.",
                        recommended_action="Avoid adding new recurring costs and keep this month's variable spending tightly controlled.",
                        source="planner",
                        metadata={"disposable_income": round(disposable, 2)},
                    )
        ...
        pace_info = budget_status.get("pace", {}) or {}
        pace_status = pace_info.get("pace", pace_info.get("status", "on_track"))

        if (
            overall_status == "warning"
            and pace_status == "ahead_of_spend"
            and percent_used >= 80
        ):
            alerts.append(
                self._make_alert(
                    ...
                    severity="critical",
                    title="Budget pressure is escalating quickly",
                    message=f"Budget use is already at {percent_used:.1f}% and spending pace is ahead of schedule.",
                    recommended_action="Switch to essentials-only spending for the rest of the week and recheck the budget.",
                    source="tracker",
                    metadata={"percent_used": percent_used, "pace": pace_info},
                )
