...
def _safe_serialize(obj, seen=None):
    ...
    if isinstance(obj, BaseModel):
        seen.add(obj_id)
        return _safe_serialize(obj.model_dump(mode="json"), seen)
...
def _as_string_list(value: Any) -> List[str]:
    """
    Normalize unknown input into a clean list[str].
    Prevents weird circular placeholders or dict/list objects from leaking into UI.
    """
    if value is None:
        return []
    ...
    if isinstance(value, list):
        ...
        for item in value:
            ...
            if isinstance(item, (str, int, float, bool)):
                ...
                safe_text = str(_safe_serialize(item)).strip()
                if safe_text and safe_text != "<circular_reference>":
    ...
    safe_text = str(_safe_serialize(value)).strip()
    if safe_text and safe_text != "<circular_reference>":
        return [safe_text]
...
class FinancialRunnerResult:
    ...
    def __init__(self, graph_output: Dict[str, Any]):
        self.raw = _safe_serialize(graph_output)

        self.pipeline_status = graph_output.get("pipeline_status", "unknown")
        self.success = self.pipeline_status != "failed"
        self.has_partial_results = self.pipeline_status == "completed_with_errors"

        self.warnings: List[str] = _as_string_list(graph_output.get("warnings", []))
        self.errors: List[str] = _as_string_list(graph_output.get("errors", []))
    ...
    def _extract_summaries(self):
        ...
        overall_health = advice_payload.get("overall_financial_health", "unknown")
        self.overall_health = str(overall_health) if overall_health is not None else "unknown"

        self.top_priorities = _as_string_list(advice_payload.get("top_priorities", []))
        self.immediate_actions = _as_string_list(advice_payload.get("immediate_actions", []))

        advisor_summary = advice_payload.get("advisor_summary", "")
        self.advisor_summary = str(advisor_summary) if advisor_summary is not None else ""

        tracking_payload = self.tracking_report.get("budget_status", {}) if isinstance(self.tracking_report, dict) else {}
        self.budget_health = str(tracking_payload.get("status", "unknown"))
        self.budget_percent_used = float(tracking_payload.get("percent_used", 0.0) or 0.0)

        alert_summary = self.alert_result.get("summary", {}) if isinstance(self.alert_result, dict) else {}
        self.alert_total = int(alert_summary.get("total_alerts", 0) or 0)
        self.alert_critical = int(alert_summary.get("critical_count", 0) or 0)
        self.alert_warning = int(alert_summary.get("warning_count", 0) or 0)
    ...
    def to_dict(self) -> Dict[str, Any]:
        payload = {
            ...
            "errors": self.errors,
            "execution_time_seconds": self.execution_time_seconds,
            "extraction_result": _safe_serialize(self.extraction_result),
            "result": {
                "transactions": [t.model_dump(mode="json") for t in self.transactions],
                "analysis": _safe_serialize(self.analysis),
                "plan": _safe_serialize(self.plan),
                "tracking_report": _safe_serialize(self.tracking_report),
                "advice_result": _safe_serialize(self.advice_result),
                "alert_result": _safe_serialize(self.alert_result),
            },
        }
        return _safe_serialize(payload)
